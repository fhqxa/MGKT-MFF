...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes_c, num_classes_f ,block=BasicBlock, fuse_type='MFF'):
        self.inplanes = 3
        ...
        self.linear_c = nn.Linear(640, num_classes_c)
        self.linear_f = nn.Linear(640, num_classes_f)
    ...
    def forward(self, x):
     ...
     # x = F.avg_pool2d(x, x.size()[3])
        ...
        x = x.view(x.size(0), -1)
        out_c = self.linear_c(x)
        out_f = self.linear_f(x)  
        return out_c, out_f


def resnet32(num_classes_c,num_classes_f):
    return ResNet(num_classes_c, num_classes_f,block=BasicBlock, fuse_type='MFF')
